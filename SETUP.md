...
| Claude Code | `project-workflow-claude` (v2.3) | `CLAUDE.md` auto-load | `~/.claude/skills/` |
...
After install, restart Claude Code or run `/reload-skills` to activate skills. The project's `CLAUDE.md` boot layer auto-loads each session. The `project-workflow-claude` v2.3 skill is fully standalone with its own workflow scripts (phase3-consensus, phase4-implement, phase5-review, phase6-verify) and does not require OMC.
...
project-workflow-claude v2.3 is fully standalone — no external dependencies required.
