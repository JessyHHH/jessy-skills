# Phase 3.5：Claude Review 后 Codex Replan

## 目标

Phase 3.5 的目标是处理 Claude Code 对 contract 的 Phase 3 审核结果。

这一步很关键：Claude Code 的 Phase 3 不是最终裁判。它是一个强审查者，但 Codex 仍然是 plan owner。
...
如果 Claude Phase 3 是 `ITERATE`：

```text
Codex 必须生成 v2
v2 必须说明采纳/拒绝了哪些建议
v2 必须再次 preflight
v2 必须再次交给 Claude Phase 3
```

如果 Claude Phase 3 是 `REJECT`：
...
这一步让你不需要监督 Claude Phase 3 是否合理，因为 Codex 会替你做 review triage。
